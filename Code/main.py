...
def write_run_information(
    ...
    with info_path.open("w", encoding="utf-8") as f:
        ...
        f.write(f"Output JSON includes reasoning fields: {include_reasoning}\n")
        f.write(f"LLM generates label reasoning: {_config_value('GENERATE_LABEL_REASONING', '<not set>')}\n")
        f.write(f"LLM generates sanitizer reasoning: {_config_value('GENERATE_SANITIZER_REASONING', '<not set>')}\n")
        ...
        f.write(f"DSPY_USE_CHAIN_OF_THOUGHT: {_config_value('DSPY_USE_CHAIN_OF_THOUGHT', '<not set>')}\n")
        f.write(f"DSPY_DISABLE_THINKING: {_config_value('DSPY_DISABLE_THINKING', '<not set>')}\n")
        f.write(f"OPTIMIZED_PROMPT_MAX_CHARS: {_config_value('OPTIMIZED_PROMPT_MAX_CHARS', '<not set>')}\n")
